 ...
 * @see       https://github.com/fellwell5/bezahlcode/
...
class Bezahlcode
{
     ...
     *
     * @param bool $use_frame (optional) Use Bezahlcode-Frame around the QRcode. Defined by the public variable $base64_frame
     *
     * @since Method available since Release 0.1
     */
    public function __construct($iban, $bic, $name, $qrprovider = 'phpqrcode', $use_frame = true)
    {
        $this->iban = $iban;
        $this->bic = $bic;
        $this->name = $name;
        ...
        if ('phpqrcode' == $qrprovider) {
            if (!file_exists(rex_path::addon('epcqrcode', 'lib/phpqrcode.php'))) {
                exit('PHPQRCODE was selected as QR-Code provider, but phpqrcode.php was not found at the specified path. Please set the variable to the path.');
            }
            if (!class_exists('QRcode')) {
                exit('PHPQRCODE was selected as QR-Code provider, but QRcode class could not be loaded.');
            }
        }
    }
